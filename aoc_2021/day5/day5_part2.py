...
for location in data:
    ...
    # Handle Vertical or Horizontals
    if x1==x2 or y1==y2:
    ...
    # Handle the Diagonals (Slope are reversed with origin in top left corner)
    elif x_distance / y_distance == -1:  
        for index, i in enumerate(range(start_x, end_x +1)):
            grid_dict[(i, end_y - index)] = grid_dict.get((i, end_y - index), 0) + 1
            
    elif x_distance / y_distance == 1:    
        for index, i in enumerate(range(start_x, end_x +1)):
            grid_dict[(i, start_y + index)] = grid_dict.get((i, start_y + index), 0) + 1
